<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('admin.roles.index')" :active="request()->routeIs('admin.roles.*')">
                        <i class="fas fa-user-tag mr-2"></i>{{ __('Rollar') }}
                    </x-nav-link>
                    @endcan

                    @can('view-permissions')
                    <x-nav-link :href="route('admin.permissions.index')" :active="request()->routeIs('admin.permissions.*')">
                        <i class="fas fa-key mr-2"></i>{{ __('Ruxsatlar') }}
                    </x-nav-link>
                    @endcan

                    @can('view-groups')
                    <x-nav-link :href="route('admin.groups.index')" :active="request()->routeIs('admin.groups.*')">
                        <i class="fas fa-users mr-2"></i>{{ __('Guruhlar') }}
                    </x-nav-link>
                    @endcan

                    @can('view-lessons')
                    <x-nav-link :href="route('admin.lessons.index')" :active="request()->routeIs('admin.lessons.*')">
                        <i class="fas fa-book mr-2"></i>{{ __('Darslar') }}
                    </x-nav-link>
                    @endcan

                    @can('view-toys')
                    <x-nav-link :href="route('admin.toys.index')" :active="request()->routeIs('admin.toys.*')">
                        <i class="fas fa-shield-alt mr-2"></i>{{ __('Qurollar') }}
                    </x-nav-link>
                    @endcan
                </div>
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @can('view-toys')
            <x-responsive-nav-link :href="route('admin.toys.index')" :active="request()->routeIs('admin.toys.*')">
                {{ __('Qurollar') }}
            </x-responsive-nav-link>
            @endcan
        </div>
